...
public struct PhotoAlbumHelper {
    ...
    public static func removeAll(from album: PHAssetCollection, completion: ((_ error: Error?) -> ())? = nil) {
    ...
    public static func save(assets: [PHAsset], to album: PHAssetCollection, completion: ((_ error: Error?) -> ())? = nil) {
        DispatchQueue.global(qos: .background).async{
            PHPhotoLibrary.shared().performChanges({
                let albumChangeRequest = PHAssetCollectionChangeRequest(for: album)
